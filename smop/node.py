...
def encode(s):
    return "".join(c + "_"
                   if c.isupper() or c == "_" else c.upper() for c in s)
...
def extend(cls, name=None):
    return lambda f: (setattr(cls, f.__name__ if name is None else name, f)
                      or f)
...
class ident(atom, recordtype("ident",
...
class try_catch(stmt, recordtype("try_catch",
                                 "try_stmt catch_stmt finally_stmt")):
    pass
...
class funcall(node, recordtype("funcall", "func_expr args nargout",
                               default=None)):
...
@extend(node, "is_const")
def node_is_const(self):
    return False


@extend(number, "is_const")
@extend(string, "is_const")
def literal_is_const(self):
    return True


@extend(expr_list, "is_const")
def expr_list_is_const(self):
    return all(t.is_const() for t in self)


@extend(matrix, "is_const")
def matrix_is_const(self):
