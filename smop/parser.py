...
from . import lexer
from .lexer import tokens, raise_exception  # noqa: F401
from . import node
from .node import exceptions
from . import options

# ident properties (set in parser.py)
# ----------------------------------
# G global
# P persistent
# A function argument
...
#
# ident properties (set in resolver.py)
# ------------------------------------
# R ref    =...a  or  =...a(b)
# D def    a=...  or   [a,b,c]=...
...
def p_expr2(p):
    ...
    if p[2] == "=":
        ...
        # resolver.py, except the following lines up
...
def p_for_stmt(p):
    ...
    for_stmt : FOR ident EQ expr SEMI stmt_list END_STMT
             | FOR LPAREN ident_init_opt RPAREN SEMI stmt_list END_STMT
             | FOR matrix EQ expr SEMI stmt_list END_STMT
    """
    if len(p) == 8:
        if not isinstance(p[2], node.ident):
            raise_exception(SyntaxError, "Not implemented: for loop",
                            p.lexer)
...
def p_persistent_stmt(p):
    ...
    """
    #                |  PERSISTENT ident_init_opt SEMI
    p[0] = node.null_stmt()
...
def p_error(p):
    if p is None:
        raise_exception(SyntaxError, "Unexpected EOF", p.lexer)
    if p.type == "COMMENT":
...
def parse(buf):
    ...
    new_lexer = lexer.new()
    global yaccparser
    yaccparser = yacc.yacc(start="top", write_tables=False,
                           debug=False
                           )
    p = yaccparser.parse(
                         buf, tracking=1, debug=options.debug_parser,
                         lexer=new_lexer)
